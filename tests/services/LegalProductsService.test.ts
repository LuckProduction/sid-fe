...
import LegalProductsService from '@/services/LegalProductsService';
import { BASE_URL } from '@/utils/api';
import { http, HttpResponse } from 'msw';
import { setupServer } from 'msw/node';
import { afterAll, beforeAll, describe, expect, it } from 'vitest';

const BASE_ENDPOINT = BASE_URL + '/legal-products';
...
describe('LegalProductsService', () => {
  beforeAll(() => server.listen({ onUnhandledRequest: 'error' }));
  afterAll(() => server.close());

  it('getAll(): should be able to get all Legal Productses', async () => {
    const { message, status, data } = await LegalProductsService.getAll('token');
  ...
  it('getAll(): should error when unauthorized', async () => {
    const { message, status, data } = await LegalProductsService.getAll('invalid token');
  ...
  it('store(): should be able to store a Legal Products', async () => {
    const { message, status } = await LegalProductsService.store(new LegalProducts(3, 'Malik'), 'token');
  ...
  it('store(): should be error when unauthorized', async () => {
    const { message, status } = await LegalProductsService.store(new LegalProducts(3, 'Malik'), 'invalid token');
  ...
  it('store(): should be error when bad request', async () => {
    const noName = await LegalProductsService.store({} as LegalProducts, 'token');
  ...
  it('update(): should be able to update a Legal Products', async () => {
    const { message, status } = await LegalProductsService.update(1, new LegalProducts(1, 'Rapik'), 'token');
  ...
  it('update(): should be error when unauthorized', async () => {
    const { message, status } = await LegalProductsService.update(1, new LegalProducts(1, 'Rapik'), 'invalid token');
  ...
  it('update(): should be error when bad request', async () => {
    const noName = await LegalProductsService.update(1, {} as LegalProducts, 'token');
    expect(noName.message).toBe('Bad request');
    expect(noName.status).toBe(false);
    expect(noName.errors).toStrictEqual({ name: ['The name field is required.'] });
  });
  ...
  it('update(): should be error when not found', async () => {
    const { message, status } = await LegalProductsService.update(2, new LegalProducts(2, 'Rapik'), 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('delete(): should be able to delete a Legal Products', async () => {
    const { message, status } = await LegalProductsService.delete(1, 'token');
  ...
  it('delete(): should be error when unauthorized', async () => {
    const { message, status } = await LegalProductsService.delete(1, 'invalid token');
  ...
  it('delete(): should be error when not found', async () => {
    const { message, status } = await LegalProductsService.delete(2, 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('deleteBatch(): should be able to delete multiple Legal Productses', async () => {
    const { message, status } = await LegalProductsService.deleteBatch([1, 2], 'token');
  ...
  it('deleteBatch(): should be error when unauthorized', async () => {
    const { message, status } = await LegalProductsService.deleteBatch([1, 2], 'invalid token');
  ...
  it('deleteBatch(): should be error when not found', async () => {
    const { message, status } = await LegalProductsService.deleteBatch([2, 3], 'token');
