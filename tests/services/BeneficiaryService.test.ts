...
const BASE_ENDPOINT = BASE_URL + '/beneficiary';
...
describe('BeneficiaryService', () => {
  ...
  it('getAll(): should be able to get all Beneficiarys', async () => {
    const { message, status, data } = await BeneficiaryService.getAll('token');
  ...
  it('getAll(): should error when unauthorized', async () => {
    const { message, status, data } = await BeneficiaryService.getAll('invalid token');
  ...
  it('store(): should be able to store a Beneficiary', async () => {
    const { message, status } = await BeneficiaryService.store(new Beneficiary(3, 'Malik'), 'token');
  ...
  it('store(): should be error when unauthorized', async () => {
    const { message, status } = await BeneficiaryService.store(new Beneficiary(3, 'Malik'), 'invalid token');
  ...
  it('store(): should be error when bad request', async () => {
    const noName = await BeneficiaryService.store({} as Beneficiary, 'token');
  ...
  it('update(): should be able to update a Beneficiary', async () => {
    const { message, status } = await BeneficiaryService.update(1, new Beneficiary(1, 'Rapik'), 'token');
  ...
  it('update(): should be error when unauthorized', async () => {
    const { message, status } = await BeneficiaryService.update(1, new Beneficiary(1, 'Rapik'), 'invalid token');
  ...
  it('update(): should be error when bad request', async () => {
    const noName = await BeneficiaryService.update(1, {} as Beneficiary, 'token');
    expect(noName.message).toBe('Bad request');
    expect(noName.status).toBe(false);
    expect(noName.errors).toStrictEqual({ name: ['The name field is required.'] });
  });
  ...
  it('update(): should be error when not found', async () => {
    const { message, status } = await BeneficiaryService.update(2, new Beneficiary(2, 'Rapik'), 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('delete(): should be able to delete a Beneficiary', async () => {
    const { message, status } = await BeneficiaryService.delete(1, 'token');
  ...
  it('delete(): should be error when unauthorized', async () => {
    const { message, status } = await BeneficiaryService.delete(1, 'invalid token');
  ...
  it('delete(): should be error when not found', async () => {
    const { message, status } = await BeneficiaryService.delete(2, 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('deleteBatch(): should be able to delete multiple Beneficiarys', async () => {
    const { message, status } = await BeneficiaryService.deleteBatch([1, 2], 'token');
  ...
  it('deleteBatch(): should be error when unauthorized', async () => {
    const { message, status } = await BeneficiaryService.deleteBatch([1, 2], 'invalid token');
  ...
  it('deleteBatch(): should be error when not found', async () => {
    const { message, status } = await BeneficiaryService.deleteBatch([2, 3], 'token');
