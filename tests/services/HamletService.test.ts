...
import HamletService from '@/services/HamletService';
import { BASE_URL } from '@/utils/api';
import { http, HttpResponse } from 'msw';
import { setupServer } from 'msw/node';
import { afterAll, beforeAll, describe, expect, it } from 'vitest';

const BASE_ENDPOINT = BASE_URL + '/hamlet';
...
describe('HamletService', () => {
  ...
  it('getAll(): should be able to get all Hamlets', async () => {
    const { message, status, data } = await HamletService.getAll('token');
  ...
  it('getAll(): should error when unauthorized', async () => {
    const { message, status, data } = await HamletService.getAll('invalid token');
  ...
  it('store(): should be able to store a Hamlet', async () => {
    const { message, status } = await HamletService.store(new Hamlet(3, 'Malik'), 'token');
  ...
  it('store(): should be error when unauthorized', async () => {
    const { message, status } = await HamletService.store(new Hamlet(3, 'Malik'), 'invalid token');
  ...
  it('store(): should be error when bad request', async () => {
    const noName = await HamletService.store({} as Hamlet, 'token');
  ...
  it('update(): should be able to update a Hamlet', async () => {
    const { message, status } = await HamletService.update(1, new Hamlet(1, 'Rapik'), 'token');
  ...
  it('update(): should be error when unauthorized', async () => {
    const { message, status } = await HamletService.update(1, new Hamlet(1, 'Rapik'), 'invalid token');
  ...
  it('update(): should be error when bad request', async () => {
    const noName = await HamletService.update(1, {} as Hamlet, 'token');
    expect(noName.message).toBe('Bad request');
    expect(noName.status).toBe(false);
    expect(noName.errors).toStrictEqual({ name: ['The name field is required.'] });
  });
  ...
  it('update(): should be error when not found', async () => {
    const { message, status } = await HamletService.update(2, new Hamlet(2, 'Rapik'), 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('delete(): should be able to delete a Hamlet', async () => {
    const { message, status } = await HamletService.delete(1, 'token');
  ...
  it('delete(): should be error when unauthorized', async () => {
    const { message, status } = await HamletService.delete(1, 'invalid token');
  ...
  it('delete(): should be error when not found', async () => {
    const { message, status } = await HamletService.delete(2, 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('deleteBatch(): should be able to delete multiple Hamlets', async () => {
    const { message, status } = await HamletService.deleteBatch([1, 2], 'token');
  ...
  it('deleteBatch(): should be error when unauthorized', async () => {
    const { message, status } = await HamletService.deleteBatch([1, 2], 'invalid token');
  ...
  it('deleteBatch(): should be error when not found', async () => {
    const { message, status } = await HamletService.deleteBatch([2, 3], 'token');
