...
import ComunityService from '@/services/ComunityService';
import { BASE_URL } from '@/utils/api';
import { http, HttpResponse } from 'msw';
import { setupServer } from 'msw/node';
import { afterAll, beforeAll, describe, expect, it } from 'vitest';

const BASE_ENDPOINT = BASE_URL + '/comunity';
...
describe('ComunityService', () => {
  ...
  it('getAll(): should be able to get all Comunitys', async () => {
    const { message, status, data } = await ComunityService.getAll('token');
  ...
  it('getAll(): should error when unauthorized', async () => {
    const { message, status, data } = await ComunityService.getAll('invalid token');
  ...
  it('store(): should be able to store a Comunity', async () => {
    const { message, status } = await ComunityService.store(new Comunity(3, 'Malik'), 'token');
  ...
  it('store(): should be error when unauthorized', async () => {
    const { message, status } = await ComunityService.store(new Comunity(3, 'Malik'), 'invalid token');
  ...
  it('store(): should be error when bad request', async () => {
    const noName = await ComunityService.store({} as Comunity, 'token');
  ...
  it('update(): should be able to update a Comunity', async () => {
    const { message, status } = await ComunityService.update(1, new Comunity(1, 'Rapik'), 'token');
  ...
  it('update(): should be error when unauthorized', async () => {
    const { message, status } = await ComunityService.update(1, new Comunity(1, 'Rapik'), 'invalid token');
  ...
  it('update(): should be error when bad request', async () => {
    const noName = await ComunityService.update(1, {} as Comunity, 'token');
    expect(noName.message).toBe('Bad request');
    expect(noName.status).toBe(false);
    expect(noName.errors).toStrictEqual({ name: ['The name field is required.'] });
  });
  ...
  it('update(): should be error when not found', async () => {
    const { message, status } = await ComunityService.update(2, new Comunity(2, 'Rapik'), 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('delete(): should be able to delete a Comunity', async () => {
    const { message, status } = await ComunityService.delete(1, 'token');
  ...
  it('delete(): should be error when unauthorized', async () => {
    const { message, status } = await ComunityService.delete(1, 'invalid token');
  ...
  it('delete(): should be error when not found', async () => {
    const { message, status } = await ComunityService.delete(2, 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('deleteBatch(): should be able to delete multiple Comunitys', async () => {
    const { message, status } = await ComunityService.deleteBatch([1, 2], 'token');
  ...
  it('deleteBatch(): should be error when unauthorized', async () => {
    const { message, status } = await ComunityService.deleteBatch([1, 2], 'invalid token');
  ...
  it('deleteBatch(): should be error when not found', async () => {
    const { message, status } = await ComunityService.deleteBatch([2, 3], 'token');
