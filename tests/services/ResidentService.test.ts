...
const BASE_ENDPOINT = BASE_URL + '/resident';
...
describe('ResidentService', () => {
  ...
  it('getAll(): should be able to get all Residents', async () => {
    const { message, status, data } = await ResidentService.getAll('token');
  ...
  it('getAll(): should error when unauthorized', async () => {
    const { message, status, data } = await ResidentService.getAll('invalid token');
  ...
  it('store(): should be able to store a Resident', async () => {
    const { message, status } = await ResidentService.store(new Resident(3, 'Malik'), 'token');
  ...
  it('store(): should be error when unauthorized', async () => {
    const { message, status } = await ResidentService.store(new Resident(3, 'Malik'), 'invalid token');
  ...
  it('store(): should be error when bad request', async () => {
    const noName = await ResidentService.store({} as Resident, 'token');
  ...
  it('update(): should be able to update a Resident', async () => {
    const { message, status } = await ResidentService.update(1, new Resident(1, 'Rapik'), 'token');
  ...
  it('update(): should be error when unauthorized', async () => {
    const { message, status } = await ResidentService.update(1, new Resident(1, 'Rapik'), 'invalid token');
  ...
  it('update(): should be error when bad request', async () => {
    const noName = await ResidentService.update(1, {} as Resident, 'token');
    expect(noName.message).toBe('Bad request');
    expect(noName.status).toBe(false);
    expect(noName.errors).toStrictEqual({ name: ['The name field is required.'] });
  });
  ...
  it('update(): should be error when not found', async () => {
    const { message, status } = await ResidentService.update(2, new Resident(2, 'Rapik'), 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('delete(): should be able to delete a Resident', async () => {
    const { message, status } = await ResidentService.delete(1, 'token');
  ...
  it('delete(): should be error when unauthorized', async () => {
    const { message, status } = await ResidentService.delete(1, 'invalid token');
  ...
  it('delete(): should be error when not found', async () => {
    const { message, status } = await ResidentService.delete(2, 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('deleteBatch(): should be able to delete multiple Residents', async () => {
    const { message, status } = await ResidentService.deleteBatch([1, 2], 'token');
  ...
  it('deleteBatch(): should be error when unauthorized', async () => {
    const { message, status } = await ResidentService.deleteBatch([1, 2], 'invalid token');
  ...
  it('deleteBatch(): should be error when not found', async () => {
    const { message, status } = await ResidentService.deleteBatch([2, 3], 'token');
