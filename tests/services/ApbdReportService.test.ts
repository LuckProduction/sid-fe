...
import ApbdReportService from '@/services/ApbdReportService';
import { BASE_URL } from '@/utils/api';
import { http, HttpResponse } from 'msw';
import { setupServer } from 'msw/node';
import { afterAll, beforeAll, describe, expect, it } from 'vitest';

const BASE_ENDPOINT = BASE_URL + '/apbd-report';
...
describe('ApbdReportService', () => {
  beforeAll(() => server.listen({ onUnhandledRequest: 'error' }));
  afterAll(() => server.close());

  it('getAll(): should be able to get all Apbd Reports', async () => {
    const { message, status, data } = await ApbdReportService.getAll('token');
  ...
  it('getAll(): should error when unauthorized', async () => {
    const { message, status, data } = await ApbdReportService.getAll('invalid token');
  ...
  it('store(): should be able to store a Apbd Report', async () => {
    const { message, status } = await ApbdReportService.store(new ApbdReport(3, 'Malik'), 'token');
  ...
  it('store(): should be error when unauthorized', async () => {
    const { message, status } = await ApbdReportService.store(new ApbdReport(3, 'Malik'), 'invalid token');
  ...
  it('store(): should be error when bad request', async () => {
    const noName = await ApbdReportService.store({} as ApbdReport, 'token');
  ...
  it('update(): should be able to update a Apbd Report', async () => {
    const { message, status } = await ApbdReportService.update(1, new ApbdReport(1, 'Rapik'), 'token');
  ...
  it('update(): should be error when unauthorized', async () => {
    const { message, status } = await ApbdReportService.update(1, new ApbdReport(1, 'Rapik'), 'invalid token');
  ...
  it('update(): should be error when bad request', async () => {
    const noName = await ApbdReportService.update(1, {} as ApbdReport, 'token');
    expect(noName.message).toBe('Bad request');
    expect(noName.status).toBe(false);
    expect(noName.errors).toStrictEqual({ name: ['The name field is required.'] });
  });
  ...
  it('update(): should be error when not found', async () => {
    const { message, status } = await ApbdReportService.update(2, new ApbdReport(2, 'Rapik'), 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('delete(): should be able to delete a Apbd Report', async () => {
    const { message, status } = await ApbdReportService.delete(1, 'token');
  ...
  it('delete(): should be error when unauthorized', async () => {
    const { message, status } = await ApbdReportService.delete(1, 'invalid token');
  ...
  it('delete(): should be error when not found', async () => {
    const { message, status } = await ApbdReportService.delete(2, 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('deleteBatch(): should be able to delete multiple Apbd Reports', async () => {
    const { message, status } = await ApbdReportService.deleteBatch([1, 2], 'token');
  ...
  it('deleteBatch(): should be error when unauthorized', async () => {
    const { message, status } = await ApbdReportService.deleteBatch([1, 2], 'invalid token');
  ...
  it('deleteBatch(): should be error when not found', async () => {
    const { message, status } = await ApbdReportService.deleteBatch([2, 3], 'token');
