...
import SubmitLetterService from '@/services/SubmitLetterService';
import { BASE_URL } from '@/utils/api';
import { http, HttpResponse } from 'msw';
import { setupServer } from 'msw/node';
import { afterAll, beforeAll, describe, expect, it } from 'vitest';

const BASE_ENDPOINT = BASE_URL + '/submit-letter';
...
describe('SubmitLetterService', () => {
  beforeAll(() => server.listen({ onUnhandledRequest: 'error' }));
  afterAll(() => server.close());

  it('getAll(): should be able to get all Submit Letters', async () => {
    const { message, status, data } = await SubmitLetterService.getAll('token');
  ...
  it('getAll(): should error when unauthorized', async () => {
    const { message, status, data } = await SubmitLetterService.getAll('invalid token');
  ...
  it('store(): should be able to store a Submit Letter', async () => {
    const { message, status } = await SubmitLetterService.store(new SubmitLetter(3, 'Malik'), 'token');
  ...
  it('store(): should be error when unauthorized', async () => {
    const { message, status } = await SubmitLetterService.store(new SubmitLetter(3, 'Malik'), 'invalid token');
  ...
  it('store(): should be error when bad request', async () => {
    const noName = await SubmitLetterService.store({} as SubmitLetter, 'token');
  ...
  it('update(): should be able to update a Submit Letter', async () => {
    const { message, status } = await SubmitLetterService.update(1, new SubmitLetter(1, 'Rapik'), 'token');
  ...
  it('update(): should be error when unauthorized', async () => {
    const { message, status } = await SubmitLetterService.update(1, new SubmitLetter(1, 'Rapik'), 'invalid token');
  ...
  it('update(): should be error when bad request', async () => {
    const noName = await SubmitLetterService.update(1, {} as SubmitLetter, 'token');
    expect(noName.message).toBe('Bad request');
    expect(noName.status).toBe(false);
    expect(noName.errors).toStrictEqual({ name: ['The name field is required.'] });
  });
  ...
  it('update(): should be error when not found', async () => {
    const { message, status } = await SubmitLetterService.update(2, new SubmitLetter(2, 'Rapik'), 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('delete(): should be able to delete a Submit Letter', async () => {
    const { message, status } = await SubmitLetterService.delete(1, 'token');
  ...
  it('delete(): should be error when unauthorized', async () => {
    const { message, status } = await SubmitLetterService.delete(1, 'invalid token');
  ...
  it('delete(): should be error when not found', async () => {
    const { message, status } = await SubmitLetterService.delete(2, 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('deleteBatch(): should be able to delete multiple Submit Letters', async () => {
    const { message, status } = await SubmitLetterService.deleteBatch([1, 2], 'token');
  ...
  it('deleteBatch(): should be error when unauthorized', async () => {
    const { message, status } = await SubmitLetterService.deleteBatch([1, 2], 'invalid token');
  ...
  it('deleteBatch(): should be error when not found', async () => {
    const { message, status } = await SubmitLetterService.deleteBatch([2, 3], 'token');
