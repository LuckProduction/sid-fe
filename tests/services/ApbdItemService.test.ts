...
import ApbdItemService from '@/services/ApbdItemService';
import { BASE_URL } from '@/utils/api';
import { http, HttpResponse } from 'msw';
import { setupServer } from 'msw/node';
import { afterAll, beforeAll, describe, expect, it } from 'vitest';

const BASE_ENDPOINT = BASE_URL + '/apbd-item';
...
describe('ApbdItemService', () => {
  beforeAll(() => server.listen({ onUnhandledRequest: 'error' }));
  afterAll(() => server.close());

  it('getAll(): should be able to get all Apbd Items', async () => {
    const { message, status, data } = await ApbdItemService.getAll('token');
  ...
  it('getAll(): should error when unauthorized', async () => {
    const { message, status, data } = await ApbdItemService.getAll('invalid token');
  ...
  it('store(): should be able to store a Apbd Item', async () => {
    const { message, status } = await ApbdItemService.store(new ApbdItem(3, 'Malik'), 'token');
  ...
  it('store(): should be error when unauthorized', async () => {
    const { message, status } = await ApbdItemService.store(new ApbdItem(3, 'Malik'), 'invalid token');
  ...
  it('store(): should be error when bad request', async () => {
    const noName = await ApbdItemService.store({} as ApbdItem, 'token');
  ...
  it('update(): should be able to update a Apbd Item', async () => {
    const { message, status } = await ApbdItemService.update(1, new ApbdItem(1, 'Rapik'), 'token');
  ...
  it('update(): should be error when unauthorized', async () => {
    const { message, status } = await ApbdItemService.update(1, new ApbdItem(1, 'Rapik'), 'invalid token');
  ...
  it('update(): should be error when bad request', async () => {
    const noName = await ApbdItemService.update(1, {} as ApbdItem, 'token');
    expect(noName.message).toBe('Bad request');
    expect(noName.status).toBe(false);
    expect(noName.errors).toStrictEqual({ name: ['The name field is required.'] });
  });
  ...
  it('update(): should be error when not found', async () => {
    const { message, status } = await ApbdItemService.update(2, new ApbdItem(2, 'Rapik'), 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('delete(): should be able to delete a Apbd Item', async () => {
    const { message, status } = await ApbdItemService.delete(1, 'token');
  ...
  it('delete(): should be error when unauthorized', async () => {
    const { message, status } = await ApbdItemService.delete(1, 'invalid token');
  ...
  it('delete(): should be error when not found', async () => {
    const { message, status } = await ApbdItemService.delete(2, 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('deleteBatch(): should be able to delete multiple Apbd Items', async () => {
    const { message, status } = await ApbdItemService.deleteBatch([1, 2], 'token');
  ...
  it('deleteBatch(): should be error when unauthorized', async () => {
    const { message, status } = await ApbdItemService.deleteBatch([1, 2], 'invalid token');
  ...
  it('deleteBatch(): should be error when not found', async () => {
    const { message, status } = await ApbdItemService.deleteBatch([2, 3], 'token');
