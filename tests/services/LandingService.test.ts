...
const BASE_ENDPOINT = BASE_URL + '/landing';
...
describe('LandingService', () => {
  ...
  it('getAll(): should be able to get all Landings', async () => {
    const { message, status, data } = await LandingService.getAll('token');
  ...
  it('getAll(): should error when unauthorized', async () => {
    const { message, status, data } = await LandingService.getAll('invalid token');
  ...
  it('store(): should be able to store a Landing', async () => {
    const { message, status } = await LandingService.store(new Landing(3, 'Malik'), 'token');
  ...
  it('store(): should be error when unauthorized', async () => {
    const { message, status } = await LandingService.store(new Landing(3, 'Malik'), 'invalid token');
  ...
  it('store(): should be error when bad request', async () => {
    const noName = await LandingService.store({} as Landing, 'token');
  ...
  it('update(): should be able to update a Landing', async () => {
    const { message, status } = await LandingService.update(1, new Landing(1, 'Rapik'), 'token');
  ...
  it('update(): should be error when unauthorized', async () => {
    const { message, status } = await LandingService.update(1, new Landing(1, 'Rapik'), 'invalid token');
  ...
  it('update(): should be error when bad request', async () => {
    const noName = await LandingService.update(1, {} as Landing, 'token');
    expect(noName.message).toBe('Bad request');
    expect(noName.status).toBe(false);
    expect(noName.errors).toStrictEqual({ name: ['The name field is required.'] });
  });
  ...
  it('update(): should be error when not found', async () => {
    const { message, status } = await LandingService.update(2, new Landing(2, 'Rapik'), 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('delete(): should be able to delete a Landing', async () => {
    const { message, status } = await LandingService.delete(1, 'token');
  ...
  it('delete(): should be error when unauthorized', async () => {
    const { message, status } = await LandingService.delete(1, 'invalid token');
  ...
  it('delete(): should be error when not found', async () => {
    const { message, status } = await LandingService.delete(2, 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('deleteBatch(): should be able to delete multiple Landings', async () => {
    const { message, status } = await LandingService.deleteBatch([1, 2], 'token');
  ...
  it('deleteBatch(): should be error when unauthorized', async () => {
    const { message, status } = await LandingService.deleteBatch([1, 2], 'invalid token');
  ...
  it('deleteBatch(): should be error when not found', async () => {
    const { message, status } = await LandingService.deleteBatch([2, 3], 'token');
