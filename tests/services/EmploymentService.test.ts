...
const BASE_ENDPOINT = BASE_URL + '/employment';
...
describe('EmploymentService', () => {
  ...
  it('getAll(): should be able to get all Employments', async () => {
    const { message, status, data } = await EmploymentService.getAll('token');
  ...
  it('getAll(): should error when unauthorized', async () => {
    const { message, status, data } = await EmploymentService.getAll('invalid token');
  ...
  it('store(): should be able to store a Employment', async () => {
    const { message, status } = await EmploymentService.store(new Employment(3, 'Malik'), 'token');
  ...
  it('store(): should be error when unauthorized', async () => {
    const { message, status } = await EmploymentService.store(new Employment(3, 'Malik'), 'invalid token');
  ...
  it('store(): should be error when bad request', async () => {
    const noName = await EmploymentService.store({} as Employment, 'token');
  ...
  it('update(): should be able to update a Employment', async () => {
    const { message, status } = await EmploymentService.update(1, new Employment(1, 'Rapik'), 'token');
  ...
  it('update(): should be error when unauthorized', async () => {
    const { message, status } = await EmploymentService.update(1, new Employment(1, 'Rapik'), 'invalid token');
  ...
  it('update(): should be error when bad request', async () => {
    const noName = await EmploymentService.update(1, {} as Employment, 'token');
    expect(noName.message).toBe('Bad request');
    expect(noName.status).toBe(false);
    expect(noName.errors).toStrictEqual({ name: ['The name field is required.'] });
  });
  ...
  it('update(): should be error when not found', async () => {
    const { message, status } = await EmploymentService.update(2, new Employment(2, 'Rapik'), 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('delete(): should be able to delete a Employment', async () => {
    const { message, status } = await EmploymentService.delete(1, 'token');
  ...
  it('delete(): should be error when unauthorized', async () => {
    const { message, status } = await EmploymentService.delete(1, 'invalid token');
  ...
  it('delete(): should be error when not found', async () => {
    const { message, status } = await EmploymentService.delete(2, 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('deleteBatch(): should be able to delete multiple Employments', async () => {
    const { message, status } = await EmploymentService.deleteBatch([1, 2], 'token');
  ...
  it('deleteBatch(): should be error when unauthorized', async () => {
    const { message, status } = await EmploymentService.deleteBatch([1, 2], 'invalid token');
  ...
  it('deleteBatch(): should be error when not found', async () => {
    const { message, status } = await EmploymentService.deleteBatch([2, 3], 'token');
