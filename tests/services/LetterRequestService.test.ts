...
import LetterRequestService from '@/services/LetterRequestService';
import { BASE_URL } from '@/utils/api';
import { http, HttpResponse } from 'msw';
import { setupServer } from 'msw/node';
import { afterAll, beforeAll, describe, expect, it } from 'vitest';

const BASE_ENDPOINT = BASE_URL + '/letter-request';
...
describe('LetterRequestService', () => {
  beforeAll(() => server.listen({ onUnhandledRequest: 'error' }));
  afterAll(() => server.close());

  it('getAll(): should be able to get all Letter Requests', async () => {
    const { message, status, data } = await LetterRequestService.getAll('token');
  ...
  it('getAll(): should error when unauthorized', async () => {
    const { message, status, data } = await LetterRequestService.getAll('invalid token');
  ...
  it('store(): should be able to store a Letter Request', async () => {
    const { message, status } = await LetterRequestService.store(new LetterRequest(3, 'Malik'), 'token');
  ...
  it('store(): should be error when unauthorized', async () => {
    const { message, status } = await LetterRequestService.store(new LetterRequest(3, 'Malik'), 'invalid token');
  ...
  it('store(): should be error when bad request', async () => {
    const noName = await LetterRequestService.store({} as LetterRequest, 'token');
  ...
  it('update(): should be able to update a Letter Request', async () => {
    const { message, status } = await LetterRequestService.update(1, new LetterRequest(1, 'Rapik'), 'token');
  ...
  it('update(): should be error when unauthorized', async () => {
    const { message, status } = await LetterRequestService.update(1, new LetterRequest(1, 'Rapik'), 'invalid token');
  ...
  it('update(): should be error when bad request', async () => {
    const noName = await LetterRequestService.update(1, {} as LetterRequest, 'token');
    expect(noName.message).toBe('Bad request');
    expect(noName.status).toBe(false);
    expect(noName.errors).toStrictEqual({ name: ['The name field is required.'] });
  });
  ...
  it('update(): should be error when not found', async () => {
    const { message, status } = await LetterRequestService.update(2, new LetterRequest(2, 'Rapik'), 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('delete(): should be able to delete a Letter Request', async () => {
    const { message, status } = await LetterRequestService.delete(1, 'token');
  ...
  it('delete(): should be error when unauthorized', async () => {
    const { message, status } = await LetterRequestService.delete(1, 'invalid token');
  ...
  it('delete(): should be error when not found', async () => {
    const { message, status } = await LetterRequestService.delete(2, 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('deleteBatch(): should be able to delete multiple Letter Requests', async () => {
    const { message, status } = await LetterRequestService.deleteBatch([1, 2], 'token');
  ...
  it('deleteBatch(): should be error when unauthorized', async () => {
    const { message, status } = await LetterRequestService.deleteBatch([1, 2], 'invalid token');
  ...
  it('deleteBatch(): should be error when not found', async () => {
    const { message, status } = await LetterRequestService.deleteBatch([2, 3], 'token');
