...
const BASE_ENDPOINT = BASE_URL + '/speech';
...
describe('SpeechService', () => {
  ...
  it('getAll(): should be able to get all Speechs', async () => {
    const { message, status, data } = await SpeechService.getAll('token');
  ...
  it('getAll(): should error when unauthorized', async () => {
    const { message, status, data } = await SpeechService.getAll('invalid token');
  ...
  it('store(): should be able to store a Speech', async () => {
    const { message, status } = await SpeechService.store(new Speech(3, 'Malik'), 'token');
  ...
  it('store(): should be error when unauthorized', async () => {
    const { message, status } = await SpeechService.store(new Speech(3, 'Malik'), 'invalid token');
  ...
  it('store(): should be error when bad request', async () => {
    const noName = await SpeechService.store({} as Speech, 'token');
  ...
  it('update(): should be able to update a Speech', async () => {
    const { message, status } = await SpeechService.update(1, new Speech(1, 'Rapik'), 'token');
  ...
  it('update(): should be error when unauthorized', async () => {
    const { message, status } = await SpeechService.update(1, new Speech(1, 'Rapik'), 'invalid token');
  ...
  it('update(): should be error when bad request', async () => {
    const noName = await SpeechService.update(1, {} as Speech, 'token');
    expect(noName.message).toBe('Bad request');
    expect(noName.status).toBe(false);
    expect(noName.errors).toStrictEqual({ name: ['The name field is required.'] });
  });
  ...
  it('update(): should be error when not found', async () => {
    const { message, status } = await SpeechService.update(2, new Speech(2, 'Rapik'), 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('delete(): should be able to delete a Speech', async () => {
    const { message, status } = await SpeechService.delete(1, 'token');
  ...
  it('delete(): should be error when unauthorized', async () => {
    const { message, status } = await SpeechService.delete(1, 'invalid token');
  ...
  it('delete(): should be error when not found', async () => {
    const { message, status } = await SpeechService.delete(2, 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('deleteBatch(): should be able to delete multiple Speechs', async () => {
    const { message, status } = await SpeechService.deleteBatch([1, 2], 'token');
  ...
  it('deleteBatch(): should be error when unauthorized', async () => {
    const { message, status } = await SpeechService.deleteBatch([1, 2], 'invalid token');
  ...
  it('deleteBatch(): should be error when not found', async () => {
    const { message, status } = await SpeechService.deleteBatch([2, 3], 'token');
