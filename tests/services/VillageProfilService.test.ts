...
import VillageProfilService from '@/services/VillageProfileService';
import { BASE_URL } from '@/utils/api';
import { http, HttpResponse } from 'msw';
import { setupServer } from 'msw/node';
import { afterAll, beforeAll, describe, expect, it } from 'vitest';

const BASE_ENDPOINT = BASE_URL + '/village-profil';
...
describe('VillageProfilService', () => {
  beforeAll(() => server.listen({ onUnhandledRequest: 'error' }));
  afterAll(() => server.close());

  it('getAll(): should be able to get all Village Profils', async () => {
    const { message, status, data } = await VillageProfilService.getAll('token');
  ...
  it('getAll(): should error when unauthorized', async () => {
    const { message, status, data } = await VillageProfilService.getAll('invalid token');
  ...
  it('store(): should be able to store a Village Profil', async () => {
    const { message, status } = await VillageProfilService.store(new VillageProfil(3, 'Malik'), 'token');
  ...
  it('store(): should be error when unauthorized', async () => {
    const { message, status } = await VillageProfilService.store(new VillageProfil(3, 'Malik'), 'invalid token');
  ...
  it('store(): should be error when bad request', async () => {
    const noName = await VillageProfilService.store({} as VillageProfil, 'token');
  ...
  it('update(): should be able to update a Village Profil', async () => {
    const { message, status } = await VillageProfilService.update(1, new VillageProfil(1, 'Rapik'), 'token');
  ...
  it('update(): should be error when unauthorized', async () => {
    const { message, status } = await VillageProfilService.update(1, new VillageProfil(1, 'Rapik'), 'invalid token');
  ...
  it('update(): should be error when bad request', async () => {
    const noName = await VillageProfilService.update(1, {} as VillageProfil, 'token');
    expect(noName.message).toBe('Bad request');
    expect(noName.status).toBe(false);
    expect(noName.errors).toStrictEqual({ name: ['The name field is required.'] });
  });
  ...
  it('update(): should be error when not found', async () => {
    const { message, status } = await VillageProfilService.update(2, new VillageProfil(2, 'Rapik'), 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('delete(): should be able to delete a Village Profil', async () => {
    const { message, status } = await VillageProfilService.delete(1, 'token');
  ...
  it('delete(): should be error when unauthorized', async () => {
    const { message, status } = await VillageProfilService.delete(1, 'invalid token');
  ...
  it('delete(): should be error when not found', async () => {
    const { message, status } = await VillageProfilService.delete(2, 'token');

    expect(message).toBe('Not found');
    expect(status).toBe(false);
  });

  it('deleteBatch(): should be able to delete multiple Village Profils', async () => {
    const { message, status } = await VillageProfilService.deleteBatch([1, 2], 'token');
  ...
  it('deleteBatch(): should be error when unauthorized', async () => {
    const { message, status } = await VillageProfilService.deleteBatch([1, 2], 'invalid token');
  ...
  it('deleteBatch(): should be error when not found', async () => {
    const { message, status } = await VillageProfilService.deleteBatch([2, 3], 'token');
