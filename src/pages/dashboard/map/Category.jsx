...
const Category = () => {
  ...
  useEffect(() => {
    fetchCategory(token, 'pemetaan', pagination.page, pagination.per_page);
            ...
            onClick={() => {
              modal.edit({
                ...
                onSubmit: async (values) => {
                  const { message, isSuccess } = await updateCategory.execute(record.id, { ...values, type: 'pemetaan' }, token);
                  if (isSuccess) {
  ...
  const onCreate = () => {
    modal.create({
      ...
      formFields: categoryFormFields,
      onSubmit: async (values) => {
        const { message, isSuccess } = await storeCategory.execute({ ...values, type: 'pemetaan' }, token);
        if (isSuccess) {
          success('Berhasil', message);
          fetchCategory(token, 'pemetaan');
        } else {
